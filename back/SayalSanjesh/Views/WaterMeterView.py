...
class WaterMeterView:
   ...
   """
    ...
    @csrf_exempt
    def admin_assign_user_view(self, request):
        try:
            input_data = json.loads(request.body)
        except:
            return result_creator(status="failure", code=406, farsi_message="وارد نشده است json",
        ...
        if "Token" in request.headers:
            ...
            token = ''
        fields = ["water_meter_serial", "water_meter_user_id"]
        for field in fields:
            if field not in input_data:
                return result_creator(status="failure", code=406, farsi_message=f".وارد نشده است {field}",
                                      english_message=f"{field} is Null.")
        ...
        result, data = WaterMeterSerializer.admin_assign_user_serializer(
            token=token, water_meter_serial=water_meter_serial, water_meter_user_id=water_meter_user_id)
